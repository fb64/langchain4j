...
public class DuckDBEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public DuckDBEmbeddingStore(String filePath, String tableName) {
        try {
            ...
            throw new RuntimeException("Unable to load duckdb connection",e);
        }
    }
    ...
    public void removeAll(final Collection<String> ids) {
        ...
        String sql = String.format("DELETE FROM %s WHERE id in (%s)", tableName,idsParam);
    ...
    public void removeAll() {
        var sql = "truncate table %s".formatted(tableName);
        try(var connection = duckDBConnection.duplicate(); var statement = connection.createStatement()){
            statement.execute(sql);
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public EmbeddingSearchResult<TextSegment> search(final EmbeddingSearchRequest request) {
        try(var connection = duckDBConnection.duplicate(); var statement = connection.createStatement()){
            ...
            var query = SEARCH_QUERY_TEMPLATE.formatted(param,tableName,Double.toString(request.minScore()),filterClause,request.maxResults());
            log.debug(query);
    ...
    private void addAllInternal(List<String> ids, List<Embedding> embeddings, List<TextSegment> embedded) {
        ...
        try (var connection = duckDBConnection.duplicate(); var statement = connection.createStatement()) {
            var values = IntStream.range(0, ids.size())
                    //.mapToObj(i-> "('%s',%s,'%s')".formatted(ids.get(i),embeddingToParam(embeddings.get(i)),embedded.get(i).text()))
                    .mapToObj(i->{
                        var text = "null";
                        ...
                        return "('%s',%s, %s, '%s')".formatted(ids.get(i),embeddingToParam(embeddings.get(i)),text, jsonMetadataSerializer.toJson(metadata));
                    })
                    .collect(Collectors.joining(","));

            var sql = insertQuery.formatted(tableName, values);
            log.debug(sql);
            statement.execute(sql);
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    private void initTable(){
        var sql = CREATE_TABLE_TEMPLATE.formatted(tableName);
        try(var connection = duckDBConnection.duplicate(); var statement = connection.createStatement()){
            log.debug(sql);
            statement.execute(sql);
        } catch (SQLException e) {
            throw new RuntimeException(String.format("Failed to init duckDB table:  '%s'", sql), e);
        }
    }
